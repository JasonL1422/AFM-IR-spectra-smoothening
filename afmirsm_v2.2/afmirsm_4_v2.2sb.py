...
num_spectra = sum('cm-1' in col for col in data.columns)  # # of colums that are cm-1
...
def plot_all_smoothed_data(smoothed_data, num_spectra):
    ...
    plt.xlabel('Wavenumber (cm-1)', fontsize=16)
    plt.ylabel('Intensity (mV)', fontsize=16)
    plt.title(f'All smth spectrum_sb_{start1,start2,start3}', fontsize=16)
    plt.legend(prop={'size':8})
    plt.gca().invert_xaxis()
    ax = plt.gca()
    ax.xaxis.set_major_locator(ticker.MultipleLocator(100))
    ax.xaxis.set_minor_locator(ticker.MultipleLocator(50))
    plt.savefig('All_smth_spectra_sb.png', dpi=300)
    plt.show()
...
def adjust_intensities(data, start):
    ...
    for i in range(num_spectra):
        ...
        if all(wavenumber in data[wavenumber_col].values for wavenumber in [before2, before1, start, target]):
            ...
            temp = (target3_intensity + target4_intensity)/2 - (before3_intensity + before4_intensity)/2
...
data.to_excel('smoothed_data_sb.xlsx', index=False)
